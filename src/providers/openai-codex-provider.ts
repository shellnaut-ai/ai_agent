...
export class OpenAICodexProvider implements ModelProvider {
  ...
  async *stream(
    ...
    // 인증을 먼저 해결해야 credential이 없을 때 model endpoint에 불필요한 요청을 보내지 않는다.
    ...
    if (!response.ok) {
      ...
      const suffix = await safeHttpErrorSuffix(response, request.model);
      throw new Error(
    ...
    for await (const data of readSseData(response.body)) {
      ...
      if (type === "response.function_call_arguments.delta") {
        ...
        yield {
          type: "tool_call_delta",
          index: requireIndex(event.output_index, "output_index"),
          argumentsDelta: requireString(event.delta, "delta"),
        };
        continue;
      }
