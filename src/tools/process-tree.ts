...
export function isNonRunnableProcessState(state: string): boolean {
  return state === "Z" || state === "X" || state === "x";
}

export const posixProcessStateRuntime: {
  readLinuxStat(pid: number): Promise<string>;
  execute(pid: number, stateSelector: string): Promise<string>;
} = {
  readLinuxStat(pid: number): Promise<string> {
    return readFile(`/proc/${pid}/stat`, "utf8");
  },
  execute(pid: number, stateSelector: string): Promise<string> {
    return executePosixProcessState(pid, stateSelector);
  },
};

function executePosixProcessState(
  pid: number,
...
function parseLinuxProcessState(pid: number, stat: string): string {
  const commandEnd = stat.lastIndexOf(")");
  if (commandEnd < 0) {
    throw new Error(`Invalid Linux process stat for PID ${pid}.`);
  }
  const state = stat.slice(commandEnd + 2).trim().split(/\s+/u)[0];
  if (state === undefined || !/^[A-Za-z]$/u.test(state)) {
    throw new Error(`Invalid Linux process state for PID ${pid}.`);
  }
  return state;
}

async function readPortablePosixProcessState(pid: number): Promise<string> {
  const errors: unknown[] = [];

  for (const selector of ["state", "stat", "s"] as const) {
    try {
      const output = await posixProcessStateRuntime.execute(pid, selector);
      const match = /^\s*([A-Za-z])/u.exec(output);
      if (match?.[1] === undefined) {
        throw new Error(`Invalid POSIX process state for PID ${pid}.`);
      }
      return match[1];
    } catch (error: unknown) {
      errors.push(error);
  ...
  throw new AggregateError(
    errors,
    "Unable to read a POSIX process with a supported state selector.",
  );
}

export async function posixProcessIsRunnable(
  pid: number,
  ...
  if (platform === "linux") {
    try {
      state = parseLinuxProcessState(
        pid,
        await posixProcessStateRuntime.readLinuxStat(pid),
      );
    } catch (error: unknown) {
    ...
    state = await readPortablePosixProcessState(pid);
  }

  return !isNonRunnableProcessState(state);
}
